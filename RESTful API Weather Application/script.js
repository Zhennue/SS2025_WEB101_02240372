...
const WEATHER_API_KEY = 'a5a4fc102605af9a5518ac9e5f1ba38d'; // Replace with your API key
...
function renderSavedLocations() {
    ...
    container.innerHTML = savedLocations.map(location =>
      `<div class="location-item" data-id="${location.id}">
        <h3>${location.name}</h3>
        <div><strong>City:</strong> ${location.country ? `<span>${location.country}</span>` : ''}</div>
        <div><strong>Notes:</strong> ${location.notes ? `<strong>Notes:</strong> ${location.notes}` : ''}</div>
